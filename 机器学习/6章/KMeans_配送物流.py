...
def myKMeans(dataSet, k):
    ...
    while clusterChanged:  # 重复计算，直到簇分配不再变化
        ...
        # 重新计算簇中心
        for cent in range(k):
            ptsInClust = dataSet[nonzero(clusterPoints[:, 0] == cent)[0]] # 选出index == cent的所有数组
            classCenter[cent, :] = mean(ptsInClust, axis=0)  # 取列项的平均值
...
def show(dataSet, k, classCenter, clusterPoints):
    ...
    imgP = plt.imread('E:/pythonProject/机器学习/datas/city.png')
    ax0.imshow(imgP)
...
dataSet = loadtxt('E:/pythonProject/机器学习/datas/testSet.txt')
K = 5  # 类的数量
